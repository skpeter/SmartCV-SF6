...
def detect_versus_screen(payload:dict, img, scale_x:float, scale_y:float):
    ...
    if core.get_color_match_in_region(img, box, (34, 7, 9), 0.05) >= 0.9:
...
ko_passes = 0
def detect_health_bars(payload:dict, img, scale_x:float, scale_y:float):
    global ko_passes
    if ko_passes < 1 and (payload['players'][0]['health'] == 0 or payload['players'][1]['health'] == 0): return
    img = img.convert("L")
    ...
    if config.getboolean('settings', 'debug_mode', fallback=False):
        print("Player 1 health bar:", healthbar1, "Player 2 health bar:", healthbar2, "KO Check:", ko_passes)
    ...
    if (np.floor(healthbar1 * 100) / 100 == 0) ^ (np.floor(healthbar2 * 100) / 100 == 0):
        ...
        if ko_passes > 1:
            ...
            if healthbar1 < healthbar2:
                ...
                return # detect_health_bars(payload, img, scale_x, scale_y)
            ko_passes = 0
            payload['round'] += 1
            if payload['players'][0]['rounds'] == 0 or payload['players'][1]['rounds'] == 0:
                ...
                if payload['state'] != previous_states[-1]:
                    previous_states.append(payload['state'])
        return
    else: ko_passes -= 1 if ko_passes > 0 else 0
    return
